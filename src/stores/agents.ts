...
export const useAgentsStore = create<AgentsState>((set) => ({
  ...
  createAgent: async (name: string, options?: Parameters<AgentsState['createAgent']>[1]) => {
    set({ error: null });
    try {
      const snapshot = await hostApiFetch<AgentsSnapshot & { success?: boolean }>('/api/agents', {
        method: 'POST',
  ...
  updateAgent: async (agentId: string, updates: Parameters<AgentsState['updateAgent']>[1]) => {
    set({ error: null });
    try {
