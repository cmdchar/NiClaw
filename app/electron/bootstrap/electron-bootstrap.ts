 ...
 */
import { app, BrowserWindow, nativeImage, session, shell, ipcMain } from 'electron';
...
import { registerZoomShortcuts } from '../main/zoom-shortcuts';

import { appUpdater, registerUpdateHandlers } from '../main/updater';
import { registerExecutionHandlers } from '../main/ipc/execution';
...
import { isQuitting, setQuitting } from '../main/app-state';
...
} from '../main/main-window-focus';
import {
  createQuitLifecycleState,
  markQuitCleanupCompleted,
  requestQuitLifecycleAction,
} from '../main/quit-lifecycle';
import { createSignalQuitHandler } from '../main/signal-quit';
import { acquireProcessInstanceFileLock } from '../main/process-instance-lock';
...
const WINDOWS_APP_USER_MODEL_ID = 'app.niclaw.desktop';
...
function verifyRuntimeIntegrity(): void {
  try {
    const manifestPath = app.isPackaged 
      ? join(process.resourcesPath, 'app.asar', 'dist', 'build-manifest.json')
      : join(__dirname, '../../dist/build-manifest.json');
      
    if (!existsSync(manifestPath)) {
      logger.warn('Runtime integrity: build-manifest.json not found, skipping validation.');
      return;
    }
    
    const manifest = JSON.parse(readFileSync(manifestPath, 'utf-8'));
    const desktopHash = manifest.artifacts?.desktop?.sha256;
    let desktopPath = manifest.artifacts?.desktop?.path;
    
    if (desktopHash && desktopPath) {
      if (app.isPackaged && desktopPath.endsWith('.ts')) {
          desktopPath = desktopPath.replace('.ts', '.js');
      }
      
      const fullPath = app.isPackaged 
        ? join(process.resourcesPath, 'app.asar', desktopPath)
        : join(__dirname, '../../', desktopPath);
        
      if (existsSync(fullPath)) {
        const fileBuffer = readFileSync(fullPath);
        const actualHash = createHash('sha256').update(fileBuffer).digest('hex');
        if (actualHash !== desktopHash) {
          logger.error(`CRITICAL: Runtime integrity validation failed for ${desktopPath}. Expected ${desktopHash}, got ${actualHash}.`);
          app.exit(1);
        } else {
          logger.info(`Runtime integrity verified for ${desktopPath}.`);
        }
      } else {
        logger.warn(`Runtime integrity: ${desktopPath} not found for validation.`);
      }
    }
  } catch (error) {
    logger.warn('Runtime integrity validation error:', error);
  }
}
...
async function initialize(): Promise<void> {
  ...
  // Validate integrity before anything else
  ...
  session.defaultSession.webRequest.onHeadersReceived(
    { urls: [`http://127.0.0.1:${getPort('OPENCLAW_GATEWAY')}/*`, `http://localhost:${getPort('OPENCLAW_GATEWAY')}/*`] },
    (details, callback) => {
