...
- Added hashed paired-device token storage in `electron/utils/android-sync.ts`; Android pairing tokens are generated by Desktop, stored locally on Android, and validated by Host API without exposing raw device tokens in Desktop storage.
- Added a Desktop Settings > Remote Access control that generates a short-lived Android pairing code through the Host API renderer proxy.
- Masked sensitive Host API query params in request logs and removed the active Android hardcoded gateway-token fallback.
- Added Android `AndroidSyncFragment.kt` and `fragment_android_sync.xml` with Host URL, pairing, Test Connection, Sync Now, heartbeat event, board, agents, and tasks sections.
- Added Android network security config for local/LAN HTTP Host API access (`10.0.2.2`, `10.10.1.219`, localhost).
- Verified Android `.\gradlew.bat assembleDebug`, Desktop `pnpm run typecheck`, and Desktop `pnpm run build:vite` successfully.
