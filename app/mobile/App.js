...
export default function App() {
  ...
  const handleConnect = async (serverConfig) => {
    setLoading(true);
    try {
      // Real API call to check Agent Mesh status
      const res = await fetch(`https://${serverConfig.url}/api/mesh/status`, {
        signal: AbortSignal.timeout(3000)
      }).catch(() => null);
      
      if (!res || !res.ok) {
        throw new Error('Agent Mesh Gateway Unreachable or API Pending');
      }
      ...
      let realNodes = [];
      if (Array.isArray(data)) realNodes = data;
      else if (data.nodes && Array.isArray(data.nodes)) realNodes = data.nodes;

      setWorkspaces(realNodes);
    } catch (e) {
