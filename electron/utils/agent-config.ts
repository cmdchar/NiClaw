...
export async function updateAgentConfig(agentId: string, options: UpdateAgentOptions): Promise<AgentsSnapshot> {
  return withConfigLock(async () => {
    ...
    if (options.name !== undefined) {
      nextEntry.name = normalizeAgentName(options.name);
    }
    if (options.description !== undefined) nextEntry.description = options.description;
    if (options.role !== undefined) nextEntry.role = options.role;
    if (options.parentAgentId !== undefined) nextEntry.parentAgentId = options.parentAgentId;
    ...
    await writeOpenClawConfig(config);
    logger.info('Updated agent config', { agentId, updates: Object.keys(options) });
    return buildSnapshotFromConfig(config);
  });
}

/** @deprecated Use updateAgentConfig */
